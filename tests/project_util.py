...
# Read input data
hospitalsDf = pd.read_csv(os.path.abspath('./data/hospitals.csv'))
colNames = list(hospitalsDf)

for n in colNames:
    if isinstance( hospitalsDf[n].values.tolist()[0], int):
        hospitalsDf = hospitalsDf.astype({n: float})
 



# Upload file
dfBytes = utl.pandas_to_bytes(hospitalsDf)

fileDoc = FileDocument()
fileDoc.name = "hospitals.csv"
fileDoc.projectId = proj.id
fileDoc.acl.owner = proj.acl.owner
fileDoc.metadata.contentEncoding = "gzip"

fileDoc = client.fileService.upload(fileDoc, dfBytes)

task = CSVTask()
task.state = InitState()
task.fileDocumentId = fileDoc.id
task.projectId = proj.id
task.owner = proj.acl.owner
...
# wkfTercen = self.context.context.client.workflowService.get("9b611b90f412969d6f617f559f005bc6")

# Create Workflow with a TableStep
wkf = Workflow()
